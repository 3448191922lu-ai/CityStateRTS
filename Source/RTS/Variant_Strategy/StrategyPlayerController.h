...
protected:
	...
	UPROPERTY()
	TArray<TObjectPtr<AStrategySquad>> ControlledSquads;
	...
	UPROPERTY()
	TArray<TObjectPtr<AStaticMeshActor>> WallPreviewActors;
...
public:
	...
	void DoIssueOrder(const FStrategyOrder& Order);
...
protected:
	...
	bool GetHitUnderCursor(FHitResult& Hit);
	...
	void SelectSquad(AStrategySquad* Squad, bool bToggle);
	void SelectBuilding(AStrategyBuilding* Building);
	void RefreshControlledUnits();
	void HandleAttackMoveKey();
	void HandleStopKey();
	void HandleBuildMenuKey();
